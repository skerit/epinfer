...
var ep = episode.addProperty('episode');

ep.match(/\Wseason\W+\d{1,2}\W+episode\W+(\d{1,2})\W/i, 90); // "season 01 episode 01"
ep.match(/s\d{1,2}e(\d{1,2})/i, 90);   // "s01e01"
ep.match(/s\d{1,2}xe(\d{1,2})/i, 90);  // "S01xE01"
ep.match(/s\d{1,2}\We(\d{1,2})/i, 80);   // "s01 e01"
ep.match(/\W\dx(\d\d)\W/, 50);         // " 1x01 "
ep.match(/\W\d\dx(\d\d)\W/, 50);       // " 01x01 "
ep.match(/\W\dx(\d)\W/, 30);           // " 1x1 "
ep.match(/\W\[\d(\d\d)\]\W/, 10);      // " [101] "
ep.match(/\W\d(\d\d)\W/, 5);           // " 101 "
