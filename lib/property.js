...
module.exports = function(Blast) {
	...
	EpinferProperty.setMethod(function castType(str) {
		...
		switch (this.type) {
			...
			case 'title':
				...
				result = result.replace(/-$/, '').replace(/^-/, '').replace(/ - /g, '.').replace(/\.|_/g, ' ').trim();
			...
			case 'language':
				...
				if (languages[str]) {
					result = languages[str].name;
				} else {
					for (i = 0; i < languages.all.length; i++) {
						temp = languages.all[i];
						if (temp.alpha2 == str) {
							result = temp.name;
							break;
						} else if (temp.name.toLowerCase() == str) {
							result = temp.name;
							break;
						}
					}
	...
	EpinferProperty.setMethod(function process(input) {
		...
		for (i = 0; i < this.matchers.length; i++) {
			...
			// Increase the score with the length of the extracted text
			if (temp.extract) {
				temp.score += temp.extract.length;
			}
	...
	var EpinferMatch = Blast.Collection.Function.inherits('Informer', function EpinferMatch(property, regex, score) {
		...
		if (regex) {
			...
			if (!regex.ignoreCase) {
				list.push(new MultiRegexp(Blast.Bound.RegExp.interpret(regex+'i')));
			}
		}
	...
	EpinferMatch.setMethod(function setRegex(regex) {
		...
		if (!regex.ignoreCase) {
			list.push(Blast.Bound.RegExp.interpret(regex+'i'));
		}
	...
	EpinferMatch.setMethod(function exec(val, modIndex, toEdge) {
		...
		if (temp) {
			...
			if (this.matcher) {
				...
				result.result = this.property.castType(this.matchValue);
