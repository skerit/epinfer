...
ep.addExtract(/\b\d\d(\d\d)\b/, 5);             // " 1101 "
ep.addExtract(/\b\d(\d\d)\b/, 5);               // " 101 "
...
subtitle.addExtract(/\bsub[\W_]([A-Z]{2,})/, 80);
...
language.addMatch('French', /\bVF\b/, 50);
...
m = series.addExtract(/(.*)/i, 11);
m.require('season');
m.positionBefore('season');
...
m = title.addExtract(/ - (.*)/i, 41);
m.require('episode');
m.positionAfter('episode');

m = title.addExtract(/ - (.*) -/i, 45);
m.require('episode');
m.positionAfter('episode');
...
// "-EVOLVE" & "-lol"
release_group.addExtract(/-([A-Z0-9\-]+)/i, 30).positionBefore('website').positionAfter('title');
